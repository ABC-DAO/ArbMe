...
export interface TradeConfig {
  ...
  alchemyKey: string;
  minEthReserve: bigint; // Minimum ETH to keep for gas (e.g., 0.0002)
  maxGasPriceGwei: number; // Max gas price we're willing to pay
  slippageTolerance: number; // e.g., 0.5% = 0.005
...
export const TOKENS = {
  ...
  ARBME: '0xC647421C5Dc78D1c3960faA7A33f9aEFDF4B7B07' as Address,
  CLANKER: '0x25bc1A101bf9D58F6036213c0e096Dfc9b5DB6EA' as Address,
...
export const POOLS = {
  ...
  CLANKER_ARBME: '0x...' as Address, // TODO: Get V4 pool ID
};
...
export interface TradeOpportunity {
  ...
  estimatedProfit: number; // USD estimate from monitoring
}
...
export interface TradeModel {
  ...
  minAmountOut: bigint; // After slippage tolerance
  ...
  gasCostWei: bigint;
  gasCostUsd: number;
  swapFeePercent: number; // 3%

  // Profit
  netProfitUsd: number; // After all costs
...
export class TradeExecutor {
  ...
  async getCurrentGasPrice(): Promise<{ gasPrice: bigint; gasPriceGwei: number }> {
  ...
  async hasEnoughEthForGas(estimatedGasCost: bigint): Promise<boolean> {
   ...
   */
  async simulateTrade(model: TradeModel): Promise<SimulationResult> {
    try {
  ...
  async executeTrade(
    ...
    tokenOut: Address,
    amountIn: bigint,
    poolKey?: any, // V4 PoolKey for actual execution
