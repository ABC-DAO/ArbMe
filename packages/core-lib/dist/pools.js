 ...
 */
function extractTokenPricesFromPools(poolGroups) {
    // Track best price per token (from highest TVL pool)
    const bestPrices = new Map();
    const updatePrice = (addr, price, tvl) => {
        if (!addr || price <= 0)
            return;
...
async function _fetchPoolsInternal(alchemyKey, cached) {
    const cacheKey = 'pools';
    try {
        ...
        if (!tokenPrices[oincAddr]) {
            ...
            if (oincPrice > 0)
                tokenPrices[oincAddr] = oincPrice;
        }
        ...
        for (const pool of [...ratchetPools, ...abcPools, ...clawdPools]) {
            const alreadyExists = allPools.some(p => p.pairAddress.toLowerCase() === pool.pairAddress.toLowerCase());
            if (!alreadyExists) {
                allPools.push(pool);
            }
        }
        ...
        const findV4WethPrice = (tokenAddr) => {
            const addr = tokenAddr.toLowerCase();
            const pool = allPools.find(p => p.dex.includes('V4') &&
