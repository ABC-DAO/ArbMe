 ...
 */
export async function getPermit2Allowance(token, owner, spender) {
 ...
 */
export function buildPermit2ApproveTransaction(token, spender, amount, expiration) {
 ...
 */
export async function checkV4Approvals(token, owner, amountRequired) {
    try {
 ...
 */
function encodeMintParams(poolKey, tickLower, tickUpper, liquidity, amount0Max, amount1Max, owner, hookData) {
    // PoolKey: 5 x 32 bytes = 160 bytes
    const poolKeyEncoded = poolKey.currency0.slice(2).toLowerCase().padStart(64, '0') +
    ...
    return `0x${poolKeyEncoded}${tickLowerEncoded}${tickUpperEncoded}${liquidityEncoded}${amount0MaxEncoded}${amount1MaxEncoded}${ownerEncoded}${hookDataOffset}${hookDataLength}${hookDataPadded}`;
}
/**
...
export function buildV4MintPositionTransaction(params) {
    ...
    const mintParams = encodeMintParams(poolKey, minTick, maxTick, liquidity, amount0, // amount0Max
    amount1, // amount1Max
    params.recipient, '0x');
    ...
    const closeCurrency0Params = encodeAbiParameters([{ type: 'address' }], [params.token0]);
    const closeCurrency1Params = encodeAbiParameters([{ type: 'address' }], [params.token1]);
    ...
    const actions = encodePacked(['uint8', 'uint8', 'uint8'], [V4_ACTIONS.MINT_POSITION, V4_ACTIONS.CLOSE_CURRENCY, V4_ACTIONS.CLOSE_CURRENCY]);
