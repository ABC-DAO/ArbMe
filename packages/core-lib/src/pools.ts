 ...
 */
async function fetchPageArbmePool(pagePrice: number, alchemyKey?: string): Promise<PoolData | null> {
  try {
    const result = await rpcCall(
      PAGE_ARBME_POOL.address,
      GET_RESERVES,
      alchemyKey
    );
    ...
    if (pageReserve === 0 || arbmeReserve === 0) return null;
 ...
 */
async function fetchOincArbmePool(oincPrice: number, alchemyKey?: string): Promise<PoolData | null> {
  try {
 ...
 */
async function fetchClankerArbmeV2Pool(clankerPrice: number, alchemyKey?: string): Promise<PoolData | null> {
  try {
    ...
    if (clankerReserve === 0 || arbmeReserve === 0) return null;
 ...
 */
async function fetchV4ArbmePools(
 ...
 */
async function fetchBalancerV3Pools(arbmePrice: number): Promise<PoolData[]> {
  try {
    const query = `{
    ...
    const data = await response.json() as BalancerPoolResponse;
    if (!data.data?.poolGetPools?.length) return [];

    const pools: PoolData[] = [];

    for (const pool of data.data.poolGetPools) {
      const tvl = parseFloat(pool.dynamicData.totalLiquidity) || 0;
      if (tvl < 10) continue; // Skip tiny pools
 ...
 */
export async function fetchPools(alchemyKey?: string): Promise<{
  token: string;
  poolCount: number;
  totalTvl: number;
  arbmePrice: string;
...
async function _fetchPoolsInternal(alchemyKey: string | undefined, cached: CacheEntry | undefined) {
  ...
  try {
    ...
    for (const balPool of balancerPools) {
      const alreadyExists = allPools.some(p =>
        p.pairAddress.toLowerCase() === balPool.pairAddress.toLowerCase()
      );
      if (!alreadyExists) {
