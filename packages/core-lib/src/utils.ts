...
function getClient(alchemyKey?: string) {
...
export async function fetchTokenBalances(
  ...
  const results = await Promise.all(
    tokens.map(token => fetchTokenBalance(token, wallet, alchemyKey))
  );
...
export async function fetchEthBalance(
  ...
  try {
    return await client.getBalance({ address: wallet as Address });
  } catch (error) {
...
export async function checkMultipleAllowances(
  ...
  const results = await Promise.all(
    tokens.map(token => checkTokenAllowance(token, owner, spender, alchemyKey))
  );
 ...
 */
export function toRawAmountWithBuffer(
 ...
 */
export function applyCustomSlippage(amount: bigint, slippagePercent: number): bigint {
 ...
 */
export function getCustomDeadline(secondsFromNow: number): bigint {
 ...
 */
export function sortAddresses(addr0: string, addr1: string): [string, string] {
 ...
 */
export function isAddressLessThan(a: string, b: string): boolean {
...
export async function withRetry<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!classified.recoverable) throw lastError;
